...
export class AddCandidateDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.candidateForm.patchValue({
      ...
      company_id: this.data.candidate.company_id !== undefined ? this.data.candidate.company_id : ''
    });
  }
  }
  ...
  onCVSelected(event: any): void {
    ...
    if (file) {
      if (file.size > 1000000) {
        this.snackBar.open('CV file size should be 1 MB or less', 'Close', { duration: 3000 });
        return;
      }
      if (file.type !== 'application/pdf') {
        this.snackBar.open('Only PDF files are allowed for CV', 'Close', { duration: 3000 });
        return;
      }
  ...
  onProfilePicSelected(event: any): void {
    ...
    if (file) {
      if (file.size > 1000000) {
        this.snackBar.open('Profile picture size should be 1 MB or less', 'Close', { duration: 3000 });
        return;
      }
      if (file.type !== 'image/jpeg') {
        this.snackBar.open('Only JPG files are allowed for profile picture', 'Close', { duration: 3000 });
        return;
      }
  ...
  const companyId = formValue.company_id === '' || formValue.company_id === null || formValue.company_id === undefined
    ? -1
  ...
  const id = (this.data.candidate && this.data.candidate.id) ? this.data.candidate.id : null;
  ...
  this.applicationsService.submit(data,id).subscribe({
    ...
    error: (error) => {
      this.snackBar.open('Error adding candidate: ' + error.message, 'Close', { duration: 3000 });
    }
  });
}
