...
import { StarRateComponent } from 'src/app/legacy/components/star-rate/star-rate.component';
import { CustomDatePipe } from 'src/app/legacy/services/custom-date.pipe';
import { RatingsService } from 'src/app/legacy/services/ratings.service';
import { RatingsEntriesService } from 'src/app/legacy/services/ratings_entries.service';
...
export class UserRatings implements OnChanges, OnInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      changes['currUserRatings'] &&
      changes['currUserRatings'].currentValue !=
        changes['currUserRatings'].previousValue
    ) {
            ...
            tooltip: {
              callbacks: {
                label: function (context: any) {
                  const labelContent = [];
                  const isCompleted = context.dataset.label || '';
                  const justification =
                    self.data[context.dataIndex].justification;

                  if (
                    context.parsed.y !== null &&
                    self.data[context.dataIndex].isNumeric
                  ) {
                    const currentValue = context.parsed.y;
                    const numericGoal = self.data[context.dataIndex].goal;
                    labelContent.push(
                      `${isCompleted}. Done: ${currentValue} | Goal: ${numericGoal}`,
                    );
                  }
                  ...
                  if (justification)
                    labelContent.push(`Justification: ${justification}`);
  ...
  getEntries() {
    this.ratingsEntriesService.getByUser(this.userData.id).subscribe((v) => {
      this.entries = v;
      this.setStarValue();
  ...
  arrangeEntries(entries: any = []) {
    ...
    const filteredEntries = entries.filter(
      (entry: any) => entry?.rating_id === selectedGoalId,
    );

    const displayData = this.datesSelection.map((date: any) => {
      return {
    ...
    displayData.forEach((element: any) => {
      const single = moment(element.date).format('ddd, MMM DD');
      ...
      filteredEntries.forEach((entry: any) => {
        ...
        if (moment(entry.date).format('YYYY-MM-DD') === element.date) {
          ...
          if (
            this.entry?.achieved &&
            entry?.amount_achieved >= this.entry?.rating.numeric_goal
          ) {
          ...
          if (!completedGoal && this.entry?.justification)
    ...
    const completedData: number[] = this.data.map((item: any) =>
      item.achieve ? item.value : 0,
    );
    const notCompletedData: number[] = this.data.map((item: any) =>
      !item.achieve ? item.value : 0,
    );
    ...
    this.ratingsService.rate({ rating }, this.selectedGoal).subscribe({
      ...
      error: (e) => {
        this.store.addNotifications(
          'There was an error rating the to do',
          'error',
        );
      },
    });
