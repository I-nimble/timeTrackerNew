...
import { ApplicationDetails } from 'src/app/legacy/components/application-details/application-details.component';
...
export class NotificationListComponent implements OnInit, AfterViewInit {
  ...
  handleClick(notification: any) {
    if (notification.type_id === 6) {
      const dialogRef = this.dialog.open(this.applicationDetailsDialog, {
        height: '500px',
        width: '600px',
        position: { top: '75px' },
        backdropClass: 'blur',
  ...
  markAsRead(notifications: Notification[]) {
    ...
    this.notificationsService.update(unreadNotifications, 2).subscribe({
      next: () => {
