...
export class ToDoFormComponent implements OnInit, OnChanges {
          ...
          this.ratingsService.getToDo(this.selectedDate).subscribe({
            next: (toDo) => {
              ...
              toDo.forEach((todoItem) => {
                const matchingItem = toDoData.find(
  ...
  checkUserToDo(): void {
    if (this.teamMemberId && this.selectedDate) {
      this.ratingsEntriesService
        .getUserGoalsByDate(this.teamMemberId!, this.selectedDate)
        .subscribe({
          next: (response) => {
  ...
  checkAchievedAmount(amount: number, i: number) {
    if (amount != null && this.toDoArray[i].numeric_goal) {
      this.toDoFormArray
        .at(i)
        .patchValue({ achieved: amount >= this.toDoArray[i].numeric_goal });
    }
  }

  handleSubmit() {
    if (this.toDoForm.valid) {
      const updatedToDoFormArray = this.toDoFormArray.value.map(
        (toDo: any) => ({
          ...toDo,
      ...
      this.ratingsEntriesService.submit(updatedToDoFormArray).subscribe({
        ...
        error: (res: any) => {
          this.store.addNotifications(
            'There was an error submitting the goal',
            'error',
          );
        },
      });
