...
import {
  DiscProfilesService,
  DiscProfile,
} from 'src/app/legacy/services/disc-profiles.service';
...
export class MatchPercentagesModalComponent implements OnInit {
    ...
    this.discProfilesService.getAll().subscribe({
      next: (profiles) => {
        ...
        if (this.data.candidate.disc_profiles) {
          this.selectedDiscProfileIds = this.data.candidate.disc_profiles.map(
            (p) => p.id,
          );
        }
      },
  ...
  loadPositionCategories(): void {
    ...
    this.matchScoresService.getPositionCategories().subscribe({
      ...
      error: (error) => {
        ...
        this.snackBar.open('Error loading position categories', 'Close', {
          duration: 3000,
        });
        this.loading = false;
      },
  ...
  loadExistingScores(): void {
    this.matchScoresService
      .getByApplicationId(this.data.candidate.id)
      .subscribe({
        next: (scores) => {
  ...
  initializeMatchScores(): void {
    this.positionCategories.forEach((category) => {
      const existingScore = this.existingScores.find(
        (score) => score.position_category_id === category.id,
      );
      this.matchScores[category.id] = existingScore
        ? existingScore.match_percentage
        : 0;
    });
  }
  ...
  hasValidScores(): boolean {
    return Object.values(this.matchScores).some(
  ...
  onSubmit(): void {
    const matchScoresData = Object.entries(this.matchScores).map(
