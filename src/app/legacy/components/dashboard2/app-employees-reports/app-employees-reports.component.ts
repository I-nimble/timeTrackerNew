...
import { AppEmployeeTableComponent } from '@legacy/pages/apps/employee/employee-table/employee-table.component';
import { CompaniesService } from '@legacy/services/companies.service';
...
export class AppEmployeesReportsComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['profile', 'workedHours', 'completedTasks'];
  ...
  ngOnInit(): void {
    ...
    this.allowedReportsManager =
      this.role === '2' && allowedReportEmails.includes(email || '');
    const today = moment();
    ...
    try {
      const sub2 = this.webSocketService
  ...
  getDataSource() {
    if (
      (this.role == '1' || this.allowedReportsManager || this.role == '4') &&
