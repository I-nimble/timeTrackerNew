...
import { Department } from 'src/app/legacy/models/Department.model';
import { Positions } from 'src/app/legacy/models/Position.model';
import { ApplicationsService } from 'src/app/legacy/services/applications.service';
import { DepartmentsService } from 'src/app/legacy/services/departments.service';
  ...
  getAllowedDepartmentIds(currentPosition: string): number[] {
    if (currentPosition === 'Virtual Assistant') {
      return this.departments
        .filter((d) =>
          ['Legal', 'Administrative', 'Customer Service and Support'].includes(
            d.name ?? '',
          ),
        )
        .map((d) => d.id!)
  ...
  applyForPosition(position: any): void {
    ...
    this.applicationsService.submit(payload, this.applicationId).subscribe({
      ...
      error: (err) => {
        ...
        this.snackBar.open('Failed to apply for position', 'Close', {
          duration: 3000,
        });
      },
    });
