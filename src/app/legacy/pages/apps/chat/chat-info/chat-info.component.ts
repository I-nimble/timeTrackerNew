...
import {
  ...
  RocketChatMessage,
  RocketChatTeam,
} from 'src/app/legacy/models/rocketChat.model';
...
export class ChatInfoComponent implements OnInit {
  ...
  canManageMembers(member: any): boolean {
    ...
    if (memberRoles.includes('owner')) {
      const ownersCount = Object.values(this.roomRoles).filter((r) =>
        r.includes('owner'),
      ).length;
  ...
  getUserAvatar(user: any): string {
    return (
      ...
      user.avatar ||
      user.profilePicture ||
      this.chatService.defaultAvatarUrl
    );
  }
  ...
  private loadAvailableUsers(): void {
    ...
    if (this.data.conversation.teamId) {
      this.chatService
        .getTeamMembers(this.data.conversation.teamId)
        .subscribe((users) => {
          this.availableUsers = users.map((u) => ({
            ...u,
            avatarUrl:
              u.avatarUrl || this.chatService.getUserAvatarUrl(u.username),
          }));
        });
    }
  }
    ...
    this.confirmationModal
      .open(ModalComponent, {
        data: {
          ...
          subject: 'room',
          message:
            'This action will permanently delete this room and all its messages.',
        },
      })
      .afterClosed()
      .subscribe((result) => {
