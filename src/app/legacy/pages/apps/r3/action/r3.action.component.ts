...
import { R3Service } from 'src/app/legacy/services/r3.service';
...
export class R3ActionComponent implements OnInit {
  ...
  private r3Service = inject(R3Service);
  ...
  getUsers(companyId?: number): Promise<void> {
    return new Promise((resolve) => {
      if (companyId) {
        this.companiesService.getEmployees(companyId).subscribe(
          (employees: any) => {
            this.users = employees.map((e: any) => e.user || e);
            this.companiesService.getEmployer(companyId).subscribe(
              (employer: any) => {
          ...
          () => {
            this.employeesService.getOrphanEmployees().subscribe(
              (orphans: any[]) => {
