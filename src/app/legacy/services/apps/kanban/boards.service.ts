...
export class BoardsService {
      ...
      return this.http
        ...
        .pipe(
          switchMap((uploadUrlRes: any) => {
            ...
            const fileName =
              uploadUrlRes.fileName || uploadUrlRes.key.split('/').pop();
  ...
  getAttachmentUrl(filename: string): Observable<any> {
    return this.http
      .get<any>(`${this.API_URI}/attachment-url/${filename}`)
      .pipe(
        catchError(() =>
          of({ url: `${environment.s3}/task_attachments/${filename}` }),
        ),
      );
  }
  ...
  addTaskToBoard(task: any): Observable<any> {
    ...
    const files: File[] = Array.isArray(task.task_attachments)
      ? task.task_attachments.filter((f: any) => f instanceof File)
      : [];
  ...
  updateTask(taskId: number, data: any): Observable<any> {
    const files: File[] = Array.isArray(data.task_attachments)
      ? data.task_attachments.filter((f: any) => f instanceof File)
      : [];
    const previousAttachments: any[] = Array.isArray(data.task_attachments)
      ? data.task_attachments.filter((f: any) => !(f instanceof File))
      : [];
    return this.uploadTaskAttachments(files).pipe(
      switchMap((uploadedFiles) => {
        ...
        return this.http.put<any>(
          `${this.API_URI}/update-task/${taskId}`,
          taskData,
        );
      }),
    );
  ...
  searchTasks(boardId: number, search: any) {
    return this.http.get<any>(
      `${this.API_URI}/search/tasks?board_id=${boardId}&q=${search}`,
    );
  }
}
