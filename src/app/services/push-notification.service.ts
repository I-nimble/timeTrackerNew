...
export class PushNotificationService {
    ...
    try {
      ...
      PushNotifications.addListener('registration', (token) => {
        ...
        try {
          if (value) {
            this.setToken(value);
            try { localStorage.setItem('pushToken', value); console.log('Persisted push token from listener'); } catch (e) {}
          } else {
  ...
  registerPushToken(token: string): Observable<any> {
    try {
      ...
      if (lastOwner && currentOwner && lastOwner !== currentOwner) {
        console.warn('Detected push token owned by another user (', lastOwner, '). Skipping delete; will re-associate token for', currentOwner);
      }
    } catch (e) {
  ...
  public async registerCurrentPushTokenIfNeeded(): Promise<void> {
    ...
    try {
      ...
      const lastOwner = localStorage.getItem('pushTokenUserId');
      if (lastOwner && currentOwner && lastOwner !== currentOwner) {
        console.warn('Ownership mismatch detected before registering current token. Will re-associate token instead of deleting.');
        try {
          if (this.token) {
