...
export class PushNotificationService {
  ...
  registerPushToken(token: string): Observable<any> {
    ...
    try {
      ...
      if (lastOwner && currentOwner && lastOwner !== currentOwner) {
        console.warn('[PushNotificationService] Detected push token owned by another user (', lastOwner, '). Cleaning up before registering for', currentOwner);
        // Attempt to delete the token on the server if we have it in memory
        if (this.token) {
          // Delete asynchronously; don't block registration function (not async).
          this.deletePushToken().toPromise()
            .then(() => {
              console.log('[PushNotificationService] Deleted old push token for previous owner.');
            })
            .catch((e) => {
              console.warn('[PushNotificationService] Could not delete old push token on server:', e);
            });
        }
        localStorage.removeItem('pushTokenUserId');
        // Clear local token so new token will be requested if needed
        this.token = null;
      }
    } catch (e) {
  ...
  deletePushToken(): Observable<any> {
    ...
    console.log('[PushNotificationService] Deleting push token:', this.token);
  ...
  async cleanupPush(): Promise<void> {
    try {
      // Attempt to delete any server-side registration for the device token.
      ...
      // Remove listeners and attempt to unregister; failures here are non-fatal.
      await PushNotifications.removeAllListeners();
      try { await PushNotifications.unregister(); } catch (e) { console.warn('[PushNotificationService] PushNotifications.unregister failed:', e); }
      console.log('[PushNotificationService] Push listeners removed and unregistered.');
    } catch (error) {
      console.error('[PushNotificationService] Error during push cleanup:', error);
    }
  }
  ...
  async registerCurrentPushTokenIfNeeded(): Promise<void> {
    ...
    try {
      ...
      const lastOwner = localStorage.getItem('pushTokenUserId');
      if (lastOwner && currentOwner && lastOwner !== currentOwner) {
        console.warn('[PushNotificationService] Ownership mismatch detected before registering current token. Cleaning up old token.');
        try {
          await this.deletePushToken().toPromise();
          console.log('[PushNotificationService] Deleted old push token due to ownership mismatch.');
        } catch (e) {
          console.warn('[PushNotificationService] Could not delete old push token on server:', e);
        }
        localStorage.removeItem('pushTokenUserId');
        // After deleting, ensure we still have a token (native layer may reissue same or new token)
        await this.ensureToken();
        if (!this.token) {
          console.log('[PushNotificationService] No token available after cleanup.');
          return;
        }
      }
    } catch (e) {
