...
export class NotificationsService {
  ...
  public submit(data: any, id: any = null) {
    ...
    if (data.selectedUsers)
      form.append('selectedUsers', JSON.stringify(data.selectedUsers));
          ...
          if (notification.users_notifications) {
            if (notification.users_notifications.user_id && notification.id) {
              ...
              return this.http.put(
                `${this.API_URI}/${notification.id}/${notification.users_notifications.user_id}`,
                body,
              );
            }
          }
    ...
    this.get().subscribe({
      next: (notifications: Notification[]) => {
        if (notifications.length > 0) {
          ...
          let recentNotifications: notificationCategory[] =
            this.notificationCategories.map((category) => ({
              ...category,
              notifications: [],
            }));
          let earlierNotifications: notificationCategory[] =
            this.notificationCategories.map((category) => ({
              ...category,
              notifications: [],
            }));
          ...
          notifications.forEach((notification) => {
            const category = this.notificationCategories.find(
              (cat) => cat.id === notification.type_id,
            );
            if (category) {
              if (
                new Date(notification.createdAt) >= twoDaysAgo &&
                new Date(notification.createdAt) <= today &&
                notification.users_notifications.status !== 2 &&
                notification.users_notifications.status !== 5
              ) {
                recentNotifications
          ...
          recentNotifications = recentNotifications.filter(
            (category) => category.notifications.length > 0,
          );
          earlierNotifications = earlierNotifications.filter(
            (category) => category.notifications.length > 0,
          );
  ...
  public getResume(
  ...
  ): Observable<{ url: SafeResourceUrl | null; extension: string | null }> {
    ...
    return this.http
      .post<Blob>(`${this.API_URI}/resume`, { applicationId }, options)
      .pipe(
        switchMap((response: Blob) => {
          ...
          const extension =
            response.type === 'application/pdf' ? 'pdf' : 'docx';
