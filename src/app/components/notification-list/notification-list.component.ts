...
@Component({
  ...
  standalone: true,
  imports: [ MaterialModule, CommonModule, MatTableModule, CdkTableModule, MatPaginatorModule,  MatCommonModule ],
...
export class NotificationListComponent implements OnInit, AfterViewInit{
  ...
  handleClick(notification: any) {
    ...
    this.notificationsService.update([notification], 2);
  }
  ...
  loadNotifications() {
    this.notificationsService.get().subscribe(notifications => {
      ...
      this.notificationsDataSource = new MatTableDataSource<any>(allNotifications);
      this.notificationsDataSource.paginator = this.paginator;
      this.loaded = true;
    });
  }
