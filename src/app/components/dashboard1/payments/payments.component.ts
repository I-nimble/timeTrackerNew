  ...
  this.employeeService.get().subscribe({
    next: (employees: any) => {
      ...
      forkJoin<any[]>(schedulesRequests).subscribe((allSchedules: any[]) => {
        ...
        forkJoin(entriesRequests).subscribe((allEntries) => {
          const entriesArray = allEntries as any[][];
          
          const workedHoursPerDay: { [day: string]: number } = {};
          const totalHoursPerDay: { [day: string]: number } = {};

          
          entriesArray.forEach((entries, idx) => {
            ...
            const employeeTotal = employeeSchedules.reduce((acc: any, schedule: any) => {
              const todayStr = moment().tz(this.companyTimezone).format('YYYY-MM-DD');
            ...
            Object.keys(employeeWorked).forEach((day) => {
              workedHoursPerDay[day] = (workedHoursPerDay[day] || 0) + employeeWorked[day];
            });
            Object.keys(employeeTotal).forEach((day) => {
              totalHoursPerDay[day] = (totalHoursPerDay[day] || 0) + employeeTotal[day];
            });
          });

          
          const workedData = days.map((day) =>
            Number(Number(workedHoursPerDay[day] || 0).toFixed(2))
          );
          ...
          const totalScheduled = days.reduce((acc, day) => acc + (totalHoursPerDay[day] || 0), 0);
